...
uint8_t reverse_8_bitGEMM(uint8_t a) {
    return ((a * 0x0802LU & 0x22110LU) | (a * 0x8020LU & 0x88440LU)) * 0x10101LU >> 16;
}

uint32_t reverse_32_bitGEMM(uint32_t a)
{
    // unsigned int __rbit(unsigned int val) // for ARM    //__asm__("rbit %0, %1\n" : "=r"(output) : "r"(input));
    return (reverse_8_bitGEMM(a >> 24) << 0) |
        (reverse_8_bitGEMM(a >> 16) << 8) |
        (reverse_8_bitGEMM(a >> 8) << 16) |
        (reverse_8_bitGEMM(a >> 0) << 24);
}

#define swap(a0, a1, j, m) t = (a0 ^ (a1 >>j)) & m; a0 = a0 ^ t; a1 = a1 ^ (t << j);

void transpose32_optimizedGEMM(uint32_t A[32]) {
    int j, k;
    unsigned m, t;
    ...
    for (j = 0; j < 16; ++j) {
        uint32_t tmp = A[j];
        A[j] = reverse_32_bitGEMM(A[31 - j]);
        A[31 - j] = reverse_32_bitGEMM(tmp);
    }
}

void transpose_32x32_bits_reversed_diagonaleGEMM(uint32_t *A, uint32_t *B, int m, int n)
{
    unsigned A_tmp[32];
    int i;
    #pragma unroll
    for (i = 0; i < 32; ++i) A_tmp[i] = A[i * m];
    transpose32_optimizedGEMM(A_tmp);
    #pragma unroll
...
unsigned char reverse_byteGEMM(unsigned char a)
{
    return ((a * 0x0802LU & 0x22110LU) | (a * 0x8020LU & 0x88440LU)) * 0x10101LU >> 16;
}
...
void transpose8rS32_reversed_diagonaleGEMM(unsigned char* A, unsigned char* B, int m, int n)
{
    unsigned x, y, t;
    ...
    B[7 * n] = reverse_byteGEMM(x >> 24);  B[6 * n] = reverse_byteGEMM(x >> 16);  B[5 * n] = reverse_byteGEMM(x >> 8);  B[4 * n] = reverse_byteGEMM(x);
    B[3 * n] = reverse_byteGEMM(y >> 24);  B[2 * n] = reverse_byteGEMM(y >> 16);  B[1 * n] = reverse_byteGEMM(y >> 8);  B[0 * n] = reverse_byteGEMM(y);
}
    ...
    const int lda, const int ldb, const int block_size)
{
    ...
    for (i = 0; i < n; i += 8) {
        ...
        for (j = 0; j < m; j += 8) {
            ...
            transpose8rS32_reversed_diagonaleGEMM(&A[a_index / 8], &B[b_index / 8], lda / 8, ldb / 8);
        }
        for (; j < m; ++j) {
    ...
    const int lda, const int ldb, const int block_size)
{
    ...
    for (i = 0; i < n; i += 32) {
        ...
        for (j = 0; j < m; j += 32) {
            int a_index = i*lda + j;
            int b_index = j*ldb + i;
            transpose_32x32_bits_reversed_diagonaleGEMM(&A[a_index / 32], &B[b_index / 32], lda / 32, ldb / 32);
            //transpose_32x32_bits_my(&A[a_index/32], &B[b_index/32], lda/32, ldb/32);
        }
        for (; j < m; ++j) {
            if (get_bit((unsigned char*)A, i*lda + j)) set_bit((unsigned char*)B, j*ldb + i);
        }
    }
}
//----------------------------
        ...
        for (c = 0; c < channels_col; ++c) {
            ...
            for (h = pad; h < height_col - pad; ++h) {
                for (w = pad; w < width_col - pad - 8; w += 8) {
                    ...
                    uint16_t *dst_ptr = (uint16_t*)&((unsigned char*)data_col)[col_index / 8];
