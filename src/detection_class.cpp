...
std::vector<std::string> objects_names_from_file(std::string const filename)
{
	...
	if (!file.is_open())
	{
		std::cout << "failed to open objects names file" << std::endl;
		return fileLines;
	}
...
DECTECTIONDLL_API Detector::Detector(std::string cfg_filename, std::string weight_filename, int gpu_id) : cur_gpu_id(gpu_id)
{
    wait_stream = 0;
    int old_gpu_index;
#ifdef GPU
...
DECTECTIONDLL_API Detector::~Detector()
{
    detector_gpu_t &detector_gpu = *static_cast<detector_gpu_t *>(detector_gpu_ptr.get());
...
DECTECTIONDLL_API int Detector::get_net_width() const {
    detector_gpu_t &detector_gpu = *static_cast<detector_gpu_t *>(detector_gpu_ptr.get());
    return detector_gpu.net.w;
}
DECTECTIONDLL_API int Detector::get_net_height() const {
    detector_gpu_t &detector_gpu = *static_cast<detector_gpu_t *>(detector_gpu_ptr.get());
    return detector_gpu.net.h;
}
DECTECTIONDLL_API int Detector::get_net_color_depth() const {
    detector_gpu_t &detector_gpu = *static_cast<detector_gpu_t *>(detector_gpu_ptr.get());
    return detector_gpu.net.c;
}


DECTECTIONDLL_API std::vector<bbox_t> Detector::detect(std::string image_filename, float thresh, bool use_mean)
{
...
DECTECTIONDLL_API image_t Detector::load_image(std::string image_filename)
{
...
DECTECTIONDLL_API void Detector::free_image(image_t m)
{
    if (m.data) {
        free(m.data);
    }
...
DECTECTIONDLL_API std::vector<bbox_t> Detector::detect(image_t img, float thresh, bool use_mean)
{
    detector_gpu_t &detector_gpu = *static_cast<detector_gpu_t *>(detector_gpu_ptr.get());
    network &net = detector_gpu.net;
    int old_gpu_index;
...
DECTECTIONDLL_API std::vector<bbox_t> Detector::tracking_id(std::vector<bbox_t> cur_bbox_vec, bool const change_history,
